...
import { ElectricityService, Transaction } from "@/types/api";
import { purchaseElectricity } from "@/lib/data";
...
export default function ElectricityCRUD({
  searchQuery,
}: TransactionsCRUDProps) {
  const [transactions, setTransactions] = useState(purchaseElectricity.getAll());
  const [selectedTransaction, setSelectedTransaction] =
    useState<ElectricityService | null>(null);
  ...
  const handleUpdate = () => {
    if (selectedTransaction?.id) {
      purchaseElectricity.update(
        String(selectedTransaction.id),
        selectedTransaction
      );
      setTransactions(purchaseElectricity.getAll());
  ...
  const handleDelete = () => {
    if (selectedTransaction?.id) {
      purchaseElectricity.delete(String(selectedTransaction.id));
      setTransactions(purchaseElectricity.getAll());
      setSelectedTransaction(null);
