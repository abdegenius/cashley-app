...
import { CalendarDays, Plus, CircleQuestionMark, PiggyBank, Wallet } from "lucide-react";
...
export default function SavingsAction({
  ...
  const [frequency, setFrequency] = useState<"once" | "weekly" | "monthly" | string>("once");
      ...
      <div className="w-full border border-border rounded-2xl p-6 space-y-4">
        ...
        <div className="space-y-1">
          ...
          <div className="w-full bg-card rounded-2xl overflow-hidden h-2">
            <div className="primary-purple-to-blue h-full" style={{ width: `${progress}%` }} />
          </div>
          <div className="w-full flex text-sm items-center justify-between">
  ...
  const renderAmountSection = () => (
    <div className="w-full space-y-4">
      ...
      <div className="w-full rounded-2xl border border-border p-6 flex items-center flex-col justify-center">
        <span className="text-3xl font-black w-full text-center placeholder-text">₦</span>
        <input
          type="number"
          value={amount}
        ...
        {numbers?.map((num) => (
          <div
            ...
            className={`rounded-4xl ${
              amount === num.toString() ? "primary-purple-to-blue p-0.5" : "bg-card"
            }`}
          >
            <button
      ...
      <div className="w-full space-y-4">
        ...
        <div className="space-y-3">
          {[
            ...
            <div
              key={freq.id}
              className={`${frequency === freq.id ? "primary-purple-to-blue" : ""} p-0.5 rounded-full`}
            >
              <div
              ...
              <div className="flex gap-3 items-center">
                ...
                <div>
                  ...
                  {method.accountNumber && (
                    <p className="text-xs text-zinc-500">{method.accountNumber}</p>
                  )}
                </div>
              </div>
  ...
  const renderInfoText = () => {
    const texts = {
      ...
      withdraw: ["Funds will be withdrawn immediately to your selected account."],
      new: [
        "Funds will be transferred immediately from your selected account.",
        "You can set up automatic transfers to reach your goal faster.",
      ],
  ...
  return (
    <div className="w-full h-full space-y-8 pb-6">
      ...
      <div className="flex gap-4 pt-4">
        {onCancel && <Button onclick={onCancel} type="primary" text="Cancel" width="flex-1" />}
        <Button onclick={handleConfirm} type="secondary" text={config.buttonText} width="flex-1" />
      </div>
    </div>
  );
}
